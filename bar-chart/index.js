...
function wrapLabel() {
  while (this.getComputedTextLength() > MAX_LABEL_WIDTH && this.textContent.length > 5) {
    this.textContent = `${this.textContent.slice(0, -4)}...`
    console.log(`new computed length ${this.getComputedTextLength()}. new length ${this.textContent.length}`)
  }
}
